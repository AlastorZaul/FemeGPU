...
const INITIAL_MOCK_DATA: ClusterApiResponse[] = [
  {
    ...
    nodes: {
      "prod-node-01": {
        ...
        reservations: [
          { namespace: 'alpha-train', application: 'jupyter-notebook', gpusRequested: 2, createdAt: new Date(Date.now() - 3600000) },
          { namespace: 'alpha-train', application: 'model-builder', gpusRequested: 1, createdAt: new Date(Date.now() - 7200000) }
        ]
      },
      "prod-node-02": {
        physical_gpus: 4, virtual_gpus: 4, reserved_gpus: 3, used_gpus: 3,
        ...
        reservations: [
          { namespace: 'beta-inference', application: 'triton-server', gpusRequested: 3, createdAt: new Date(Date.now() - 600000) }
        ]
      }
    }
  },
  {
    ...
    nodes: {
      ...
      "staging-node-22": {
        ...
        reservations: [
          { namespace: 'dev-test', application: 'tensorboard', gpusRequested: 1, createdAt: new Date(Date.now() - 86400000) }
        ]
      }
    }
  }
